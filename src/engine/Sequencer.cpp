/*
    Sequencer.cpp - This file is part of BTV
...
namespace Element {
    ...
    trackNode (const TrackModel& t) {
        return dynamic_cast<Element::GraphProcessor::Node*> (
    ...
    Sequencer::getNodeFor (const ValueTree& state)
    {
        ...
        NodePtr ptr = dynamic_cast<Element::GraphProcessor::Node*> (state.getProperty ("node", var::null).getObject());
    ...
    Sequencer::prepareToPlay (double sampleRate, int blockSize)
    {
        ...
        Element::GraphProcessor::prepareToPlay (sampleRate, blockSize);
    }

    void
    Sequencer::releaseResources()
    {
        Element::GraphProcessor::releaseResources();
    }
