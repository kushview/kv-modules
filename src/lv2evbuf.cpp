...
void
lv2_evbuf_toggle_flow (LV2_Evbuf* evbuf, bool input)
{
    switch (evbuf->type) {
    case LV2_EVBUF_EVENT:
        assert (false);
        break;
    case LV2_EVBUF_ATOM:
        evbuf->buf.atom.atom.type = input ? evbuf->atom_Sequence : evbuf->atom_Chunk;
        break;
    }
}
...
void
lv2_evbuf_set_size (LV2_Evbuf* evbuf)
{
...
namespace Element {
    ...
    void
    LV2Evbuf::removeUntil (uint32 frame)
    {
#if 0
        // TODO: evbuf event removal;
        LV2_Evbuf_Iterator iter = findFirst (*this, frame, 0);
        uint32_t offset  = iter.offset;
        uint32_t newSize = lv2_evbuf_pad_size (sizeof (LV2_Atom_Sequence_Body) + (uint32_t) getSize() - offset);

        LV2_Atom_Sequence* aseq = (LV2_Atom_Sequence*) getBuffer();
        LV2_Atom_Sequence_Body* body = &aseq->body;
        (uint8_t*) memmove (aseq, aseq + offset, ((LV2_Atom*)&aseq->body)->size);

        assert (aseq->atom.type == evbuf->atom_Sequence);
        aseq->atom.size = newSize;

        LV2_ATOM_SEQUENCE_FOREACH (aseq, ev)
        {
            if (ev->time.frames >= frame)
               ev->time.frames -= frame;
            else
               ev->time.frames = 0;
            std::cout << "seq move: " << ev->time.frames << std::endl;
        }
#endif
    }
