...
JackClient::close()
{
    return_value_if_null (client, "JACK server not running");
...
JackClient::registerPort (const String& name, const String& type, int flags,
                           int bufferSize)
{
    return_value_if_null (client, JackPort (*this));
...
JackClient::getName()
{
    return_value_if_null (client, String());
