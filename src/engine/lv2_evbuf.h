...
#define LV2_EVBUF_FOREACH(buf, iter) \
for (LV2_Evbuf_Iterator iter = lv2_evbuf_begin (buf); \
     iter.offset != lv2_evbuf_end(buf).offset; \
...
/**
   Return the buffer's total capacity in bytes
*/
uint32_t
...
/**
   Insert an event at frame/subframe
 */
bool
lv2_evbuf_insert (LV2_Evbuf*     evbuf,
                  uint32_t       frame,
                  uint32_t       subframe,
                  uint32_t       type,
                  uint32_t       size,
