...
public:
    ...
    void mouseDown (const MouseEvent& e)
    {
        ...
        if (e.mods.isPopupMenu())
        {
            ...
            menu.addItem (4, "Show all parameters");
...
//==============================================================================
GraphEditorPanel::GraphEditorPanel (GuiApp& app_, GraphController& graph_)
    : gui (app_), graph (graph_)
{
...
void GraphEditorPanel::mouseDown (const MouseEvent& e)
{
    if (e.mods.isPopupMenu())
    {
        ...
        {
            KnownPluginList& plugs (gui.world().plugins().allPlugins());
            plugs.addToMenu (m, KnownPluginList::sortByCategory);

            m.addItem (1, "Audio Output Device");
            int res = m.show();
...
FilterComponent*
GraphEditorPanel::getComponentForFilter (const uint32 filterID) const
{
    for (int i = getNumChildComponents(); --i >= 0;)
    {
...
ConnectorComponent*
GraphEditorPanel::getComponentForConnection (const Arc& arc) const
{
    for (int i = getNumChildComponents(); --i >= 0;)
    {
